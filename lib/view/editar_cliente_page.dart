...
import 'package:pdd/repository/cliente_repository.dart';
...
//OBTER ANTIGO - SEM REST
  /*void _obterCleinte() async {
    this._cleinte = Cleinte(this._id, "Cleinte ${this._id}", "Raça", 10);
    _nomeController.text = this._cleinte!.nome;
    _racaController.text = this._cleinte!.raca;
    _idadeController.text = this._cleinte!.idade.toString();
  }*/
...
//CRUD | editar_cleinte_page.dart//DAO
/*
  void _obterCleinte() async {
    Database db = await ConennectionFactory.factory.database;
    CleinteDAO dao = CleinteDAO(db);
    this._cleinte = await dao.obterPorId(this._id);

    ConennectionFactory.factory.close();

    _nomeController.text = this._cleinte.nome;
    _nomeController.text = this._cleinte.raca;
    _nomeController.text = this._cleinte.idade.toString();

    try {
      ClienteRepository repository = ClienteRepository();
      this._cleinte = await repository.buscar(this._id);
      _nomeController.text = this._cleinte!.nome;
      _racaController.text = this._cleinte!.raca;
      _idadeController.text = this._cleinte!.idade.toString();
    } catch (exception) {
      showError(context, "Erro recuperando cleinte", exception.toString());
      Navigator.pop(context);
    }
  }
*/
//SALVAR ANTIGO - SEM REST
  /* void _salvar() async {
    this._cleinte!.nome = _nomeController.text;
    this._cleinte!.raca = _racaController.text;
    this._cleinte!.idade = int.parse(_idadeController.text);

    ScaffoldMessenger.of(context)
        .showSnackBar(SnackBar(content: Text('Cleinte editado com sucesso.')));
  }
  */
